 ...
 * ATmega microprocessors with 8 kB of flash memory and a TWI module.
 *
...
static uint8_t write_buffer[I2C_DATA_LENGTH];
static uint8_t read_buffer[I2C_DATA_LENGTH];

static struct i2c_m_sync_desc si5351_sync;
static struct _i2c_m_msg si5351_msg;
 ...
 *
 */
void si5351_init(struct i2c_m_sync_desc* const WIRE_I2C, uint8_t xtal_load_c, uint32_t ref_osc_freq)
{
	si5351_sync = *WIRE_I2C;
	i2c_m_sync_enable(&si5351_sync);
	i2c_m_sync_set_slaveaddr(&si5351_sync, SI5351_BUS_BASE_ADDR, I2C_M_SEVEN);
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	// Clear write buffer
	memset(write_buffer, 0, I2C_DATA_LENGTH);

	// Set first byte of write buffer to Si5351 reg address
	write_buffer[0] = addr;

	// Set rest of write buffer to the data
	memcpy((write_buffer + 1), data, bytes);

	/* Init i2c packet. */
	si5351_msg.addr		= SI5351_BUS_BASE_ADDR;
	si5351_msg.len		= bytes + 1;
	si5351_msg.flags	= I2C_M_STOP;
	si5351_msg.buffer	= write_buffer;
	
	// Write buffer to slave until success.
	while (_i2c_m_sync_transfer(&si5351_sync.device, &si5351_msg))
	{
...
uint8_t si5351_write(uint8_t addr, uint8_t data)
{
	...
	// Set second byte of write buffer to data
	write_buffer[1] = data;
...
uint8_t si5351_read(uint8_t addr, uint8_t *data)
{
	...
	while (i2c_m_sync_cmd_read(&si5351_sync, addr, read_buffer, 1))
	{
		// Increment timeout counter and check if timed out.
		if (timeout++ == I2C_TIMEOUT)
		{
