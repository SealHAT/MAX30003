...
unsigned long Switch_Condition;
...
{    UPLF_Value = analogRead(UPLF_PIN);
    ...
    data = NormalRead(CNFG_GEN);
    if(UPLF_Value<LOW_THRES){
      Switch_Condition = 0;
      }else{
      Switch_Condition = 1;
      }
      Serial.print(Switch_Condition);
    if(((data>>19)&0b1)!=Switch_Condition){
      MAX30003_switch(Switch_Condition);
      }
